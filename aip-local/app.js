...
function render() {
  ...
  q.options.forEach((opt, i) => {
    ...
    // On reveal, show why each option is correct / incorrect.
    if (isRev && q.optionExplanations && q.optionExplanations[i]) {
      const ex = document.createElement("div");
      ex.className = "opt-explain " + (q.correct.includes(i) ? "ok" : "no");
      renderRich(ex, q.optionExplanations[i]);
      form.appendChild(ex);
    }
  });
  ...
  if (isRev) {
    ...
    fb.innerHTML = `<span class="verdict">${rec.correct ? "✓ Correct" : "✗ Incorrect"}</span><span class="explain"></span>`;
    renderRich(fb.querySelector(".explain"), q.explanation || "");
    if (q.reference && q.reference.url) {
      const a = document.createElement("a");
      a.className = "ref-link";
      a.href = q.reference.url;
      a.target = "_blank";
      a.rel = "noopener noreferrer";
      a.textContent = "↗ Why this is correct — AWS docs: " + (q.reference.title || q.reference.url);
      fb.appendChild(a);
    }
  } else {
    fb.hidden = true;
...
// Render text that may contain ```lang fenced code blocks. Prose keeps its
// newlines (CSS white-space) and code renders in a monospace <pre><code>.
